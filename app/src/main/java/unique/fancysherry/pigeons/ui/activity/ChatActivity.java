...
public class ChatActivity extends ToolbarCastActivity
    ...
    @Override
    public void setData() {
        List<Message> cacheMessageList = new Select()
                .from(Message.class)
                .where("message_from = ?" + " AND " + "message_to = ?"
                        ...
                        , current_username)
                .execute();
        if (cacheMessageList.size() != 0) {
            messageList.addAll(cacheMessageList);
    ...
    public void insertMessage(Message message) {
        ...
        pMessage.gid = message.gid;
        pMessage.type = message.type;
        pMessage.message = message.message;
        pMessage.from = message.from;
        pMessage.to = message.to;
        pMessage.save();
    }
